...
// to be printed to standard output. Pretty-print a bitboard for debugging.
...
Bitboard sliding_attack(PieceType pt, Square sq, Bitboard occupied) 
    ...
    static const Direction RookDirections[4]   = {NORTH, SOUTH, EAST, WEST};
    static const Direction BishopDirections[4] = {NORTH_EAST, SOUTH_EAST, SOUTH_WEST, NORTH_WEST};
	...
	{
	    Square s = sq;
        while ((s = safe_destination(s, directions[i])) != 0) 
		{
		    attacks |= s;
            if (occupied & s) break;
...
void init_magics(PieceType pt, Bitboard table[], Magic magics[][2]) {
    ...
    for (Square s = SQ_A1; s <= SQ_H8; ++s) 
        ...
        do 
            ...
            if (HasPext) 
				m.attacks[pext(b, m.mask)] = reference[size - 1];
            b = (b - m.mask) & m.mask;
        } while (b);

#ifndef USE_PEXT
        PRNG rng(8977 + 5731 * rank_of(s)); // Optimized seeding
      // Find a magic for square 's' picking up an (almost) random number
      // until we find the one that passes the verification test.
        for (int i = 0; i < size;) 
            for (m.magic = rng.sparse_rand<Bitboard>(); popcount((m.magic * m.mask) >> 56) < 6;)
